...
testdata = {  # Test ID -> (Location args, expected string representation, sort order)
    ...
    "abs_path_cell": ((Path("/foo/bar"), 2), os.path.join(os.sep, "foo", "bar[2]"), 221),
    ...
    "abs_path_cell_line": ((Path("/foo/bar"), 2, 3), os.path.join(os.sep, "foo", "bar[2]:3"), 222),
    "rel_path_cell_line": ((Path("foo"), 3, 4), "foo[3]:4", 322),
    # Cell number is omitted for non-notebooks.
    "no_path_line": (("<stdin>", None, 2), "<stdin>:2", 112),
    "abs_path_line": ((Path("/foo/bar"), None, 3), os.path.join(os.sep, "foo", "bar:3"), 212),
